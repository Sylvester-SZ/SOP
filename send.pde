...
void sendCollectedDatamouse() {
  try {
    // Konverter ArrayList til JSON-streng
    String json = convertToJSONmouse(mouseData);
    println("JSON til sending: " + json);
    
    // once per send
    
    // HTTP POST-anmodning
    String urlString = "https://sylvesterz.pythonanywhere.com/mousedata/";
    URL url = new URL(urlString);
    HttpURLConnection conn = (HttpURLConnection) url.openConnection();
    conn.setRequestMethod("POST");
    conn.setRequestProperty("Content-Type", "application/json; utf-8");
...
String convertToJSON(ArrayList<HashMap<String, Integer>> mouse) {
  // Byg JSON-streng manuelt
  StringBuilder json = new StringBuilder("[");
  for (int i = 0; i < mouse.size(); i++) {
    HashMap<String, Integer> entry = mouse.get(i);
    json.append("{");
    ...
    json.append("\"missionskaerm\":").append(entry.get("missionskaerm")).append(",");
    json.append("\"userId\":").append(entry.get("userId")).append(",");
    json.append("\"kontal\":").append(entry.get("kon")).append(",");
    json.append("\"alder\":").append(entry.get("alder")).append(",");
    json.append("\"scrollPos\":").append(entry.get("scrollPos")).append(",");
    json.append("\"skaerm\":").append(entry.get("skaerm"));
    json.append("}");
    if (i < mouse.size() - 1) json.append(",");
  }
  json.append("]");
...
String convertToJSONmouse(ArrayList<HashMap<String, Integer>> data) {
  // Byg JSON-streng manuelt
  StringBuilder json = new StringBuilder("[");
  for (int i = 0; i < data.size(); i++) {
    HashMap<String, Integer> entry = data.get(i);
    json.append("{");
    json.append("\"mouseX\":").append(entry.get("mouseX")).append(",");
    json.append("\"realmouseY\":").append(entry.get("realmouseY")).append(",");
    json.append("\"programtime\":").append(entry.get("programtime")).append(",");
    json.append("\"mission\":").append(entry.get("mission")).append(",");
    json.append("\"missionstart\":").append(entry.get("missionstart")).append(",");
    json.append("\"missionskaerm\":").append(entry.get("missionskaerm")).append(",");
